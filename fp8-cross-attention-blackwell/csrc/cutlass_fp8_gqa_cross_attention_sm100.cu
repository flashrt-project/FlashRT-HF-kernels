...
using Mainloop = cutlass::fmha::collective::Sm100FmhaFwdMainloopTmaWarpspecialized<
    ...
    StrideQ, StrideK, StrideV, cutlass::fmha::collective::ResidualMask>;
...
extern "C" int flashrt_fp8_gqa_cross_attention_sm100(
    ...
    float* lse, void* workspace, size_t workspace_capacity,
    ...
    cudaStream_t stream)
{
    ...
    if (wsz > workspace_capacity) {
        printf("[FP8 FMHA] workspace too small (%zu > %zu)\n",
               wsz, workspace_capacity);
        return -11;
    }
    if (op.initialize(args, workspace, stream) != cutlass::Status::kSuccess) return -2;
