...
__global__ void silu_mul_to_nvfp4_swizzled_kernel(
  ...
  for (int i = threadIdx.x; i < cols; i += blockDim.x) {
    ...
    float silu_g_bf = __bfloat162float(__float2bfloat16(silu_g));
    __nv_bfloat16 silu_mul_bf = __float2bfloat16(silu_g_bf * u);
    smem_silu_mul[i] = silu_mul_bf;

    float val = fabsf(__bfloat162float(silu_mul_bf));
    int blk = i >> 4;
...
__global__ void silu_mul_merged_to_nvfp4_swizzled_grouped_kernel(
  ...
  if (local < kElemsPerCta && block_col < num_blocks) {
    ...
    smem_abs[local] = fabsf(__bfloat162float(silu_mul_bf));
  }
  __syncthreads();
  ...
  for (int pass = 0; pass < 2; ++pass) {
    ...
    if (block_col < num_blocks) {
      ...
      float silu_g = silu_f32(g);
        ...
        smem_abs[idx] = fabsf(__bfloat162float(silu_mul_bf));
    }
  }
  __syncthreads();
