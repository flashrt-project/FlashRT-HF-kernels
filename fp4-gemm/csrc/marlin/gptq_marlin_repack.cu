...
int repack_nvfp4_to_marlin_sm120(const void* qweight_kn, void* out,
                                 int size_k, int size_n,
                                 cudaStream_t stream) {
  if (qweight_kn == nullptr || out == nullptr || size_k <= 0 ||
      size_k % marlin::tile_k_size != 0 || size_n <= 0 ||
  ...
  int max_shared_mem = props.sharedMemPerBlockOptin;
  ...
  marlin::gptq_marlin_repack_kernel<marlin::repack_threads, 4, false, false>
      <<<props.multiProcessorCount, marlin::repack_threads, max_shared_mem,
         stream>>>(reinterpret_cast<const uint32_t*>(qweight_kn), nullptr,
                   reinterpret_cast<uint32_t*>(out), size_k, size_n);
