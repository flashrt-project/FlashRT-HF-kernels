#!/usr/bin/env python3
"""Minimal HF Hub-style FP8 SwiGLU MLP block example."""
...
def main() -> None:
    ...
    if get_kernel is not None:
        ...
        import flashrt_fp8_swiglu_ffn as ops

    x_scale = torch.tensor([0.05], device="cuda", dtype=torch.float32)
    gate_up_scale = torch.tensor([0.04], device="cuda", dtype=torch.float32)
    ...
    gate_up_w = quantize_fp8(
        torch.randn((2 * args.h, args.k), device="cuda", dtype=torch.bfloat16),
        gate_up_scale,
    )
    down_w = quantize_fp8(
        torch.randn((args.n, args.h), device="cuda", dtype=torch.bfloat16),
        down_scale,
    )

    fn = ops.fp8_swiglu_mlp_bf16
    if args.compile:
        fn = torch.compile(fn, fullgraph=True, mode="reduce-overhead")

    y = fn(x, gate_up_w, down_w, x_scale, gate_up_scale, hidden_scale, down_scale)
    torch.cuda.synchronize()
    mode = "torch.compile(fullgraph=True)" if args.compile else "eager"
    print(f"{mode} fp8_swiglu_mlp_bf16 output shape={tuple(y.shape)} dtype={y.dtype}")
