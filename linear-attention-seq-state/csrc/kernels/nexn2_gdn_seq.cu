...
#pragma unroll 16
  for (int i = 0; i < HD; ++i)
    col[i] = __bfloat162float(state[state_h_off + (size_t)i * HD + t]);

  const float inv_sqrt_hd = rsqrtf(static_cast<float>(HD));

  for (int ts = 0; ts < S; ++ts) {
    const size_t qkv_off = ((size_t)ts * num_v_heads + h) * HD + t;
    qs[t] = __bfloat162float(q_in[qkv_off]);
    ks[t] = __bfloat162float(k_in[qkv_off]);
    __syncthreads();
    ...
    const float g_t = __expf(__bfloat162float(g_in[ts * num_v_heads + h]));
    const float beta_t = __bfloat162float(beta_in[ts * num_v_heads + h]);
...
#pragma unroll 16
    ...
    const float v_t = __bfloat162float(v_in[qkv_off]);
