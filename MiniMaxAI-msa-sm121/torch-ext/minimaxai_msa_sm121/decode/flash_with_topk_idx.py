...
def flash_decode_with_topk_idx(
    ...
    if use_dense_main_attn:
        # VENDOR NOTE: this fast path needs the un-vendored CUDA JIT kernel
        # `sglang.jit_kernel.minimax_decode_topk` (minimax_decode_topk.cuh). The
        # standalone vendor only ships the pure-Triton path, so this is gated off
        # by default (use_dense_main_attn=False). If you opt in, vendor that .cuh
        # too. Original line: `from sglang.jit_kernel.minimax_decode_topk import (
        # minimax_decode_topk_page_table)`.
        raise NotImplementedError(
            "use_dense_main_attn=True requires the un-vendored CUDA JIT kernel "
            "sglang.jit_kernel.minimax_decode_topk.minimax_decode_topk_page_table; "
            "the standalone Triton vendor only supports use_dense_main_attn=False."
        )
    ...
    if _skip_block_topk:
        ...
        # -1 padded); ~2-16x faster for long context.
        # VENDOR NOTE: this needs the un-vendored CUDA JIT kernel
        # `sglang.jit_kernel.minimax_decode_topk`. It is gated off by the
        # `_compat.envs` stub (SGLANG_OPT_USE_MINIMAX_DECODE_TOPK_RADIX=False), so
        # this branch is never taken in the standalone vendor (the 2-stage Triton
        # `else` fallback below runs instead). Original line:
        # `from sglang.jit_kernel.minimax_decode_topk import minimax_decode_topk`.
        raise NotImplementedError(
            "use_jit_topk requires the un-vendored CUDA JIT kernel "
            "sglang.jit_kernel.minimax_decode_topk; the standalone Triton vendor "
