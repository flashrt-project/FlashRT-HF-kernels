...
def _gqa_share_sparse_decode_kernel(
    ...
    # q is always needed; load it unconditionally.
    ...
    # Sink init under a PURE-constexpr `if HAS_SINK:` so Triton dead-code-
    # eliminates the sink_ptr reference when HAS_SINK is False (sink_ptr is
    # then None). The original compound `if HAS_SINK and pid_c == 0` traced
    # the body even when False -> NoneType make_block_ptr compile error on
    # sm_121. M3 has no sink (sparse_init_block=0), so HAS_SINK is always
    # False for us, but this keeps the sink path correct too.
    if HAS_SINK:
        if pid_c == 0:
            ...
            sink = tl.load(
                sink_ptrs, boundary_check=(0, 1), padding_option="zero"
            ).to(tl.float32)
