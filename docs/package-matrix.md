...
| `fp4-gemm` | `quantize_fp4_sfa_fp16`, `dequantize_fp4_sfa_fp16`, `nvfp4_gemm_bf16` | `csrc/gemm/fp4`, `csrc/quantize` | PyTorch GEMM over the same dequantized FP4/SFA/SFB inputs | Native Blackwell NVFP4 A4W4 GEMM package |
| `weight-only-ffn` | W4A16/W8A16 quantize/dequantize, linear, SwiGLU, GeGLU, and GELU FFN regions | small-M W4A16 FlashRT kernels plus package-local W8 and fusion paths | PyTorch eager and numerically equivalent warmed `torch.compile` over the same dequantized weights | True BF16-activation weight-only decode path with performance-qualified auto dispatch |
...
| `sageattention2-blackwell` | `sage2_prefill_f16_bf16_d128`, `sage2_prefill_fp8v_bf16_d128`, `sage2_qk_int8_sv_*_bf16_d128`, Q/K/V quant helpers | `csrc/attention/sage2` | PyTorch SDPA BF16 reference | Blackwell SageAttention2-style prefill attention for Wan non-causal and Qwen causal GQA shapes |
| `fp8-prefill-attention-blackwell` | `fp8_causal_gqa_attention_bf16` | `csrc/attention/fmha_fp8_causal_gqa_sm120.*` | BF16 eager/compiled SDPA over the exact FP8 values plus FP32 correctness reference | Native FP8 causal GQA prefill for tuned Blackwell long-sequence rows |
| `grouped-moe-gemm` | `grouped_nvfp4_gemm_bf16` | `csrc/kernels/moe_{m16,m64,blocktile}_mma_sm120.*` | Explicit NVFP4 dequantize plus expert-selected FP32 matmul | M16/M64 grouped W4A4 MoE prefill GEMM |
| `smallm-ffn-megakernels-blackwell` | `fp8_gelu_ffn_gated_residual_bf16_static`, `fp8_gelu_ffn_residual_bf16_static` | `csrc/kernels/megakernel/*ffn_megakernel*sm120.*` | Eager and compiled operation-by-operation quantized region references | Complete graph-friendly small-M FP8 FFN regions |
...
| Native FP4 runtime path | `fp4-fused-ops`, `fp4-gemm` | FP16-to-NVFP4 producers, FP4-to-FP4 combiners, and NVFP4 A4W4 GEMM |
| Small-M weight-only FFN | `weight-only-ffn` | BF16-activation W4A16/W8A16 linear and complete FFN regions for qualified decode shapes |
| FP8 KV attention | `fp8-kv-attention` | BF16-query XQA over FP8 E4M3 paged K/V cache for long-context decode/verify |
| SageAttention2 prefill attention | `sageattention2-blackwell` | QK-int8/SV-fp16 or SV-fp8 prefill attention for Wan/video self-attn and Qwen causal GQA long prompts |
...
- FP8 KV attention: v1 artifact target is CUDA 12.8+ Blackwell `sm_120`
  fixed-shape XQA. Do not claim `sm_121` or non-Blackwell architectures until
  a matching artifact build and runtime validation pass exists.
