...
| `flashrt-vla-video` | `q_norm_rope_bf16`, `k_norm_rope_v_cache_bf16`, then `residual_rmsnorm_quant_nvfp4`, `silu_mul_quant_nvfp4`, `video_conv_lowbit` | `csrc/kernels`, `flash_wm/csrc`, `csrc/conv`, `csrc/quantize` | PyTorch eager, FlashRT internal reference, model-block baseline | First 30x-class showcase package |
...
1. `flashrt-gemm-epilogues`: ship first because the format is proven. Public
   ...
   epilogues.
2. `flashrt-vla-video`: strongest first showcase. The first implemented Q/K
   decode post-processing slice has internal 30x-class benchmark evidence.
