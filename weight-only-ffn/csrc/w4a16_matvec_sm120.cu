...
//
// Small-M W4A16 GEMV (NVFP4 weight x BF16 activation). See header.
...
// One warp owns one output column and reuses each packed weight block across
// all M activation rows. Staging an M x 512 activation tile keeps shared-memory
// use bounded while avoiding M independent kernel launches and M weight reads.
template <int Rows, int Warps, int TileK>
__global__ void w4a16_smallm_kernel(
    const __nv_bfloat16* __restrict__ x,
    const uint8_t* __restrict__ W,
    const uint8_t* __restrict__ SFB,
    __nv_bfloat16* __restrict__ out,
    float alpha, int N, int K, int n_col_super) {
  extern __shared__ __nv_bfloat16 x_tile[];
  const int lane = threadIdx.x & 31;
  const int row = blockIdx.x * Warps + (threadIdx.x >> 5);
  const int K_BLOCKS = K >> 4;
  const int blocks_per_tile = TileK >> 4;
  const int rb = row >> 7;
  const int ri = row & 127;
  const int rb_ncs = rb * n_col_super;
  const int row_inner = (ri & 31) * 16 + ((ri >> 5) & 3) * 4;
  const uint64_t* w_blk = row < N
      ? reinterpret_cast<const uint64_t*>(W + static_cast<size_t>(row) * (K >> 1))
      : nullptr;
  float accum[Rows] = {};

  for (int tile_block = 0; tile_block < K_BLOCKS;
       tile_block += blocks_per_tile) {
    const int tile_blocks = min(blocks_per_tile, K_BLOCKS - tile_block);
    const int tile_elems = tile_blocks * 16;
    for (int idx = threadIdx.x; idx < Rows * tile_elems; idx += blockDim.x) {
      const int m = idx / tile_elems;
      const int col = idx - m * tile_elems;
      x_tile[idx] = x[static_cast<size_t>(m) * K + tile_block * 16 + col];
    }
    __syncthreads();

    if (row < N) {
      for (int local_block = lane; local_block < tile_blocks; local_block += 32) {
        const int kb = tile_block + local_block;
        const uint64_t packed = w_blk[kb];
        const float sf = c_ue4m3[
            __ldg(SFB + sf_off(rb_ncs, row_inner, kb))] * alpha;
#pragma unroll
        for (int m = 0; m < Rows; ++m) {
          const auto* xb2 = reinterpret_cast<const __nv_bfloat162*>(
              x_tile + static_cast<size_t>(m) * tile_elems + local_block * 16);
          accum[m] += blockdot(packed, xb2) * sf;
        }
      }
    }
    __syncthreads();
...
#pragma unroll
    for (int off = 16; off > 0; off >>= 1) {
      value += __shfl_xor_sync(0xffffffff, value, off);
    }
    if (lane == 0) out[static_cast<size_t>(m) * N + row] =
        __float2bfloat16(value);
  }
}

template <int Rows, int Warps, int TileK>
void launch_smallm(const void* x_bf16, const void* W_packed,
                   const void* SFB, void* out, int N, int K, float alpha,
                   int n_col_super, cudaStream_t stream) {
  const size_t smem = static_cast<size_t>(Rows) * TileK * sizeof(__nv_bfloat16);
  w4a16_smallm_kernel<Rows, Warps, TileK>
      <<<(N + Warps - 1) / Warps, Warps * 32, smem, stream>>>(
...
int w4a16_smallm_sm120_bf16(
    const void* x_bf16, const void* W_packed, const void* SFB, void* out,
    int M, int N, int K, float alpha, int variant, cudaStream_t stream) {
  if (!x_bf16 || !W_packed || !SFB || !out) return 1;
  if (M <= 0 || M > kSmallMMax || N <= 0 || K <= 0 || (K & 15) != 0) {
    return 2;
  }
  init_ue4m3_lut();
  const int n_col_super = ((K >> 4) + 3) / 4;
#define LAUNCH_ROWS(R) \
  do { \
    if (variant == 2) launch_smallm<R, 4, 256>(x_bf16, W_packed, SFB, out, N, K, alpha, n_col_super, stream); \
    else launch_smallm<R, 8, 512>(x_bf16, W_packed, SFB, out, N, K, alpha, n_col_super, stream); \
  } while (0)
  switch (M) {
