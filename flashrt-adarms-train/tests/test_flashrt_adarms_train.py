...
def run(ops, mode):
    ...
    for b,t,h in shapes:
        ...
        torch.autograd.gradcheck(lambda a,m: ops.adarms(a,m,1e-6,True)[0], (x,mod), eps=1e-6, atol=1e-4, rtol=1e-3)
        y,gate=ops.adarms(x,mod)
        ...
        (y.float().square().mean()+gate.float().square().mean()).backward(); count+=1
        ...
        def fn(a,bb,cc,dd): return ops.resgate_adarms(a,bb,cc,dd)[1]
        with torch.autograd.set_detect_anomaly(True): checkpoint(fn, xb,hb,gb,mb, use_reentrant=False).float().sum().backward()
        torch.compile(lambda a,bb,cc,dd: ops.resgate_adarms(a,bb,cc,dd)[1], fullgraph=False)(xb.detach(),hb.detach(),gb.detach(),mb.detach()); count+=1
