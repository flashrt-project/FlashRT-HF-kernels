...
compiling the other architecture's implementation. The SM110 operation is an
explicit experimental API: it passes correctness on Thor but loses to cuDNN on
the measured Cosmos3-Edge sites, so it is never selected by default.
